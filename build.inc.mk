...
# Reproducible builds: SOURCE_DATE_EPOCH for __DATE__/__TIME__ and other tools
# respecting it (https://reproducible-builds.org/docs/source-date-epoch/#makefile).
# Set to the date of the last commit.
ifndef SOURCE_DATE_EPOCH
SOURCE_DATE_EPOCH := $(or $(shell git -C $(PROJ_DIR) log -1 --pretty=%ct 2>/dev/null),0)
endif
...
# patsubst is anchored at the start and must match whole word.
rel = $(patsubst $(PROJ_DIR)/%,%,$(patsubst $(BUILD_DIR)/%,%,$(abspath $(1))))

# Resolve path under PROJ_DIR
# Lets source lists be shorthanded project-relative (src/main.c) rather than $(PROJ_DIR)/src/main.c
# Absolute paths outside the project are unchanged (e.g. /usr/include/... )
...
# The translation always appends to prevent collision on identically named files
...
# Each phase runs as a sequential sub-makes, so each phase can never race an earlier one. Each phase can be parallelized with -j.
build::
